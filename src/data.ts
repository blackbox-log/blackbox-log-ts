...
import type { DataParserInfo, ManagedPointer, Wasm, WasmObject } from './wasm';
...
export class DataParser implements WasmObject, IterableIterator<ParserEvent> {
	...
	readonly #info: DataParserInfo;

	/** @internal */
	constructor(
		wasm: Wasm,
		ptr: ManagedPointer<DataParser>,
		info: DataParserInfo,
		headers: LogHeaders,
	) {
		this.#wasm = wasm;
	...
	next(): IteratorResult<Readonly<ParserEvent>> {
		...
		const value = this.#wasm.dataNext(this.#ptr.ptr, this.#info);
